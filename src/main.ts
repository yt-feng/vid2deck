...
const URL_IMPORT_ENABLED = String(import.meta.env.VITE_URL_IMPORT_ENABLED || '').toLowerCase() === 'true';
const SCREEN_RECORDING_ENABLED = String(import.meta.env.VITE_SCREEN_RECORDING_ENABLED || '').toLowerCase() === 'true';
...
app.innerHTML = `
  <main id="homeView">
    <section class="hero">
      <div>
        ...
        <p class="subhead">支持批量上传视频和图片。单个视频可进入工作台精修；图片可生成 PPTX；批量上传后，一键生成所有页面并下载 Frames ZIP。</p>
      </div>
    </section>
    ...
    <section class="panel video-panel">
      <div class="panel-heading">
        <div>
          ...
          <h2>上传视频抽帧，生成 PPT/PDF</h2>
        </div>
        <span>完成后自动进入处理工作台</span>
      </div>

      <form id="videoUrlForm" class="url-import" ${URL_IMPORT_ENABLED ? '' : 'hidden aria-hidden="true"'}>
        <label>B站/在线视频链接
      ...
      <label class="dropzone" id="dropzone" for="videoInput" tabindex="0">
      ...
      <div class="source-actions" ${SCREEN_RECORDING_ENABLED ? '' : 'hidden aria-hidden="true"'}>
  ...
  <main id="workspaceView" class="workspace" hidden>
    ...
    <section class="workspace-main">
      ...
      <section class="workspace-command-bar" aria-label="上传视频">
        <form id="workspaceVideoUrlForm" class="workspace-url-import" ${URL_IMPORT_ENABLED ? '' : 'hidden aria-hidden="true"'}>
          <label>在线视频链接
        ...
        <div class="workspace-import-actions">
          <button id="workspaceRecordScreenBtn" type="button" ${SCREEN_RECORDING_ENABLED ? '' : 'hidden'}>录制屏幕</button>
      ...
      <section class="workspace-body">
        ...
        <section class="workspace-grid-wrap">
          <div id="workspaceEmptyState" class="workspace-empty-state">
            ...
            <p id="workspaceEmptyBody">上传视频后，处理进度和可导出的页面会出现在这里。</p>
          </div>
  ...
  <dialog id="urlFallbackDialog" class="url-fallback-dialog" ${URL_IMPORT_ENABLED ? '' : 'hidden aria-hidden="true"'}>
...
async function downloadVideoFromUrl(mode: UrlDownloadMode): Promise<void> {
  if (!URL_IMPORT_ENABLED) {
    setUrlDownloadStatus('在线视频链接功能当前未开放，请直接上传视频文件。', 'error');
    return;
  }
  if (isBusy()) return;
...
async function startWorkspaceUrlDownload(mode: UrlDownloadMode): Promise<void> {
  if (!URL_IMPORT_ENABLED) {
    setStatus('在线视频链接功能当前未开放，请直接上传视频文件。');
    return;
  }
...
function addWorkspaceFiles(files: File[]): void {
  ...
  if (selectedFile && slides.length === 0) {
    setStatus(`已加入：${selectedFile.name}。点击“开始当前任务”即可生成页面。`);
...
function returnHomeForVideoStart(): void {
  ...
  setHomeStatus('在这里上传视频文件，处理后会自动进入工作台。');
}
...
function updateActionState(): void {
  ...
  workspaceRecordScreenBtn.disabled = busy || !limits.screen_recording || !SCREEN_RECORDING_ENABLED;
  workspaceRecordScreenBtn.hidden = !SCREEN_RECORDING_ENABLED;
  workspaceStopRecordBtn.hidden = !SCREEN_RECORDING_ENABLED || !isRecording;
  imageInput.disabled = busy;
  notebookPdfInput.disabled = busy;
  recordScreenBtn.disabled = busy || !limits.screen_recording || !SCREEN_RECORDING_ENABLED;
  recordScreenBtn.hidden = !SCREEN_RECORDING_ENABLED;
  stopRecordBtn.hidden = !SCREEN_RECORDING_ENABLED || !isRecording;
...
function updateWorkspaceEmptyState(): void {
  ...
  workspaceEmptyBody.textContent = '上传视频后，生成进度和可导出的页面会出现在这里。';
  emptyWorkspaceStartBtn.textContent = '上传视频';
  emptyWorkspaceStartBtn.disabled = false;
}
